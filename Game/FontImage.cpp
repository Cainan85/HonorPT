...
static int DrawFontBackToTexture2D( SFontImageChInfo *lpFontInfo, float fPos_x, float fPos_y, DWORD rgba, float fRatio )
{
	...
	if( lpD3DDevice != NULL )
	{
		lpD3DDevice->SetTexture( 0, smMaterial[ s_iFontImageMatNum ].smTexture[0]->lpD3DTexture );
		lpD3DDevice->DrawPrimitive( D3DPT_TRIANGLESTRIP, D3DFVF_TLVERTEX, tlVertex, 4, NULL );
	}
...
void CreateFontImage(void)
{
	...
	FILE *fp = nullptr;
	fopen_s(&fp, "image\\Bellatra\\FontImagePos.dat", "rb");
	if( fp )
	{
