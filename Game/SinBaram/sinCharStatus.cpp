...
std::vector<int> TempStatePoints = std::vector<int>();
...
void cCHARSTATUS::Main()
{
	...
	{ //캐릭터 스테이터스가 닫혀있으면 보정할수 있는 능력수치 초기화 
		TempStatePoints.clear();
	}
			...
			if (sinChar->StatePoint > 9)
			{
				...
				switch (PointButtonPosi)
				{
					case 1: //힘
						...
						for (int i = 0; i < 10; i++)
						{
							TempStatePoints.push_back(POINT_STRENGTH);
						}
						ReformCharForm(); //재인증 
					...
					case 2: //정신력 
						...
						for (int i = 0; i < 10; i++)
						{
							TempStatePoints.push_back(POINT_SPIRIT);
						}
						ReformCharForm(); //재인증 
						cInvenTory.SetItemToChar();
						cInvenTory.CheckDamage(); //서버에 최고데미지를 보낸다
					...
					case 3: //재능 
						...
						for (int i = 0; i < 10; i++)
						{
							TempStatePoints.push_back(POINT_TALENT);
						}
						ReformCharForm(); //재인증 
						cInvenTory.SetItemToChar();
					...
					case 4: //미첩성 
						...
						for (int i = 0; i < 10; i++)
						{
							TempStatePoints.push_back(POINT_DEXTERITY);
						}
						ReformCharForm(); //재인증 
						cInvenTory.SetItemToChar();
						cInvenTory.CheckDamage(); //서버에 최고데미지를 보낸다
					...
					case 5: //건강 
						...
						for (int i = 0; i < 10; i++)
						{
							TempStatePoints.push_back(POINT_HEALTH);
						}
						ReformCharForm(); //재인증 
						cInvenTory.SetItemToChar();
						cInvenTory.CheckDamage(); //서버에 최고데미지를 보낸다
		...
		else
		{
			if (sinChar->StatePoint > 0)
			{
				...
				switch (PointButtonPosi)
				{
					...
					case 6:
					{
						if (TempStatePoints.size() < 1)
							break;

						int LastPoint = TempStatePoints[TempStatePoints.size() - 1];
...
void cCHARSTATUS::CheckLevel()
{
	...
	if (sinChar->Level > OldLevel)
	{
		...
		if ((sinQuest_levelLog & QUESTBIT_LEVEL_80_2) == QUESTBIT_LEVEL_80_2)
		{
			...
			{
				if (sinChar->Level >= 90)
				{ //퀘스트를 수행하고난후에는 포인트가 10씩올라간다
					sinChar->StatePoint += 3;
				}
			}
			if (sinChar->Level >= 80)
			{ //퀘스트를 수행하고난후에는 포인트가 7씩올라간다
