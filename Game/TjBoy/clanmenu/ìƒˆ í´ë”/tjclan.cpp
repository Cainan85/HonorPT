...
{	
	...
	if(vrunRuning == 1) 
	{
		extern VOICTALKUSERDATA voiceUser;
		extern BOOL bMic; //¸¶ÀÌÅ© on,off
		char buf[256];

		if(bMic == 0) {
			wsprintf(buf, "[ voice Chat Å×½ºÆ®Áß À½¼ºOFF ]");
		}
		else {
			wsprintf(buf, "[ voice Chat Å×½ºÆ®Áß À½¼ºON ]");
		}
	...
	char Msg3[1024] = {0,};
	int nPos= 0;
//	if(g_CtrlProcess.GetClanWonList_Ptr() != NULL)
	{
