...
int rsTRANS_SERVER::RecvMessage(smTHREADSOCK *pData)
{
	...
	smPLAYDATA* lpPlayData1 = nullptr;
	smTRANS_COMMAND TransCommand = { 0 };
	smTRANS_COMMAND_EX TransCommandEx = { 0 };
	...
	TRANS_POST_ITEM* lpTransPostItem = nullptr;

	DWORD	dwTime = 0;
	DWORD	dwDataKey = 0;
	smCHAR_INFO	*lpCharInfo_MsgBack = nullptr;
	int addExp = 0, addMoney = 0;
	DWORD dwEncCode = 0;
	...
	if (MessageLoading)
	{
		...
		if (!RecvDataQue[cnt])
		{
			...
			if (RecvDataQue[cnt])
			{
				memcpy_s(RecvDataQue[cnt], sizeof(smTHREADSOCK),pData, sizeof(smTHREADSOCK));
				RecvDataQuePush++;
				return TRUE;
			}
		}
	...
	{
		smTHREADSOCK smEncData = { 0 };

		memcpy_s(&smEncData, sizeof(smTHREADSOCK), pData, sizeof(smTHREADSOCK) - smSOCKBUFF_SIZE);
		...
		{
			memcpy_s(lpCharInfo_MsgBack, sizeof(smCHAR_INFO), &lpCurPlayer->smCharInfo, sizeof(smCHAR_INFO));
		}
	}
	else
	{
	...
	switch (IPData[1])
	{
		...
		case smTRANSCODE_WHISPERMESSAGE:
			...
			{ //&& lpChar->smCharInfo.State!=smCHAR_STATE_USER ) {
		...
		case smTRANSCODE_ITEM_EXPRESS:
			...
			*/
			...
			{ //받을 아이템이 있을경우 공간을 체크한후 메세지박스를띄운다
...
DWORD	dwFuncList[][2] = {
	...
	{ (DWORD)Code_VRamBuffOpen << 1			,	(DWORD)3600	},
	{ (DWORD)0								,	(DWORD)0 }
};
