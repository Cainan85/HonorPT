...
// 렌더링을 위한 연산과 정렬
// Add alignment operation for rendering
int smSTAGE3D::RenderGeom()
{
	...
	//Initializes a temporary buffer for rendering (vertex pointer will tell)
	...
	for (int h = MapPosiTop; h<MapPosiBot; h++)
	{
		...
		{
			clipW = w & 0xFF;
			if (StageArea[clipW][clipH])					//Rendering is the area
			{
				num = StageArea[clipW][clipH][0];				//Be included in the zone with Facelist
				...
				for (cnt = 1; cnt<num + 1; cnt++)
				{
					...
					{
						rendface = smRender.AddStageFace(face);			//Add to Facelist rendering
					}
					k++;
				}
			}
	...
	smRender.ClipRendFace();				// Clipping rendering the entire face
	smRender.GeomVertex2D();				// Vertices in the 2D coordinate transformation
