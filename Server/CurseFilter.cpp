...
int LoadCurses(const char *filename)
{
	...
	while (fgets(buf, sizeof(buf), file)) {
		...
		ptr += wsprintfA(ptr, passcode);

		//ptr += sprintf_s(ptr, buf);
	...
	while (ptr = getField(ptr, '\n', line), line[0]) {
		...
		while (cptr = strstr(string, curse), cptr) {
			...
			if ((int)(max_len) <
				...
				strncpy_s(result, string, cptr - string);
				lstrcpyA(result + (cptr - string), cptr + strlen(curse));
			}
			else {
				strncpy_s(result, string, cptr - string);
				lstrcpyA(result + (cptr - string), replace);
				lstrcpyA(result + (cptr - string + strlen(replace)), cptr + strlen(curse));
			}
