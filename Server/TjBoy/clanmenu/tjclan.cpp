...
//====================================================================
//ktj : Server.suoÆÄÀÏ¶§¹®¿¡ ¼¼ÀÌºê°¡¾ÈµÇ´Â°æ¿ì°¡ ÀÖÀ¸¹Ç·Î ±×·²¶§´Â ±× ÆÄÀÏÀ» Áö¿ï°Í
//====================================================================
//5.7 P4.28
...
void CLAN_MENU::CLANmenu_INIT()
{
	...
	//SetJoinData("ÇÏÇÏÇÏ","¿ïÆ®¶ó","ÇåÆ®");
	//Å¬·£ÀåÀ¸·Î µî·ÏµÈ idÀÓ			
	strcpy_s(szConnServerName,"Æë±Ï¼­¹ö");
...
int CLAN_MENU::menu_joinOpen(char *chipCharID)		//¯…ÀÇ Ä³¸¯ÅÍ id
{
	...
	strcpy_s(cldata.chipCharID, chipCharID);
...
int CLAN_MENU::IsCheckClanMember(smCHAR *pChar)
{
	...
	//strcpy_s(cldata.ChaName, pChar->smCharInfo.szName);
...
jmp1:
	...
	if(joinOK==0) {
		...
		*/
		...
		strcpy_s(cldata.clanJoinID, clanWonID);
		strcpy_s(cldata.clanJoinCharID, clanWonCharID);
...
{	
	...
	if(vrunRuning == 1) 
	{
		...
		extern BOOL bMic; //¸¶ÀÌÅ© on,off
		char buf[256];

		if(bMic == 0) {
			wsprintf(buf, "[ voice Chat Å×½ºÆ®Áß À½¼ºOFF ] ½ºÀ§Ä¡ shift+tab");
		}
		else {
			wsprintf(buf, "[ voice Chat Å×½ºÆ®Áß À½¼ºON ] ½ºÀ§Ä¡ shift+tab");
		}
		...
		while(cnt<8) {
			...
			wsprintf(buf, "¸»ÇÑÀ¯Àú %s",voiceUser.uID[cnt]);
...
{
	strcpy_s(cldata.szJoinClanName,    joinclanname);			//°¡ÀÔµÉ Å¬·£ ÀÌ¸§
	strcpy_s(cldata.szJoinClanChipName,joinclanchip);		//°¡ÀÔµÉ Å¬·£ÀÇ Å¬·£Àå id
}
